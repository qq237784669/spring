...
public abstract class AbstractBeanDefinitionReader implements BeanDefinitionReader, EnvironmentCapable {
	 ...
	 *从指定的资源位置加载bean定义。
	 * <p>该位置也可以是位置模式，前提是
	 *此bean定义读取器的ResourceLoader是ResourcePatternResolver。
	 * @param定位资源位置，加载ResourceLoader
	这个bean定义阅读器的*（或ResourcePatternResolver）
	 * @param actualResources一个Set用实际的Resource对象填充
	 *已在装载过程中解决。 可能是{@code null}
	 *表示调用者对这些Resource对象不感兴趣。
	 * @return找到的bean定义数
	...
	public int loadBeanDefinitions(String location, @Nullable Set<Resource> actualResources) throws BeanDefinitionStoreException {
		...
		// 采用classpathConfigApplication 走的是这个  是子类
		if (resourceLoader instanceof ResourcePatternResolver) {
			...
			// 这种模式是  如果  加载 配置文件的模式是 url 模式 就远程 获取 资源文件  相当于把配置文件 放到远端 加载的时候 获取
